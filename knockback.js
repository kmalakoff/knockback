...
(function() {
  ...
  */
  ...
  kb.Factory = (function() {
    ...
    function Factory(path, factory) {
      this.paths = {};
    }

    Factory.prototype.isEmpty = function() {
      return _.isEmpty(this.paths);
    };

    Factory.prototype.addPathMapping = function(path, create_fn) {
      return this.paths[path] = create_fn;
    };
    ...
    Factory.prototype.createForPath = function(obj, path, store) {
      var create_info, options;
      options = {
        store: store,
        factory: this,
        path: path
      };
      create_info = this.paths[path];
      if (!create_info) {
        return kb.Factory.createDefault(obj);
      }
      if (_.isFunction(create_info)) {
        return new create_info(obj, options);
      }
      if (create_info.create) {
        return create_info.create(obj, options);
      }
    };

    Factory.createDefault = function(obj) {
  ...
  */
  ...
  kb.Store = (function() {
    ...
    Store.prototype.destroy = function() {
      var index, view_model, _ref, _ref1;
      this.objects = null;
      _ref = this.view_models;
      for (index in _ref) {
        view_model = _ref[index];
        if (!kb.utils.observableInstanceOf(view_model, kb.CollectionObservable)) {
          continue;
        }
        this.view_models[index] = null;
        while (view_model.refCount() > 0) {
          view_model.release();
        }
      }
      _ref1 = this.view_models;
      for (index in _ref1) {
        view_model = _ref1[index];
        if (!view_model) {
          continue;
        }
        this.view_models[index] = null;
      ...
      return this.view_models = null;
    };

    Store.prototype.registerValue = function(obj, view_model) {
      var index;
      if (view_model instanceof kb.RefCountable) {
        view_model.retain();
      }
      index = _.indexOf(this.objects, obj);
      if (index >= 0) {
        this.view_models[index] = view_model;
      } else {
        this.objects.push(obj);
        this.view_models.push(view_model);
      }
      return view_model;
    };

    Store.prototype.resolveValue = function(obj, factory, path) {
      var index, view_model;
      index = _.indexOf(this.objects, obj);
      if (index >= 0) {
        if (this.view_models[index]) {
          if ((this.view_models[index] instanceof kb.RefCountable) && (this.view_models[index].refCount() <= 0)) {
            this.view_models[index] = null;
          } else {
            if (this.view_models[index] instanceof kb.RefCountable) {
              return this.view_models[index].retain();
            } else {
              return this.view_models[index];
            }
          }
        }
      } else {
    ...
    Store.prototype.releaseValue = function(view_model) {
      var index;
      if (!(view_model instanceof kb.RefCountable)) {
        return;
      }
      view_model.release();
      if (view_model.refCount() > 0) {
        return;
      }
      index = _.indexOf(this.view_models, view_model);
      if (!(index >= 0)) {
        return;
      }
      return this.view_models[index] = 0;
    };

    Store.prototype.addResolverToOptions = function(options, obj) {
    ...
    Store.resolveFromOptions = function(options, view_model) {
      if (!(options.store && options.store_obj)) {
        return;
      }
      return options.store.registerValue(options.store_obj, view_model);
    };
  ...
  kb.CollectionObservable = (function(_super) {
    ...
    function CollectionObservable(collection, options) {
      ...
      if (options.view_model) {
        this.__kb.factory.addPathMapping('models', options.view_model);
      } else if (options.create) {
        this.__kb.factory.addPathMapping('models', {
    ...
    CollectionObservable.prototype.hasViewModels = function() {
      return !this.__kb.factory.isEmpty();
    };
    ...
    CollectionObservable.prototype._createTarget = function(model) {
      if (this.hasViewModels()) {
        return this.__kb.store.resolveValue(model, this.__kb.factory, 'models');
      } else {
        return model;
      }
    };
  ...
  */
  ...
  kb.AttributeObservable = (function() {
    ...
    function AttributeObservable(model, key, options) {
      ...
      this.__kb.key = key;
      this.__kb.options = _.clone(options);
      observable = kb.utils.wrappedObservable(this, ko.dependentObservable({
    ...
    AttributeObservable.prototype.update = function(new_value) {
      ...
      if (!this.__kb.value_observable) {
        this.__kb.value_observable = this.__kb.options.factory ? this.__kb.options.factory.createForPath(new_value, this.__kb.key, this.__kb.options.store) : kb.Factory.createDefault(new_value);
        return;
      }
      ...
      } else if (kb.observableInstanceOf(this.__kb.value_observable, kb.CollectionObservable)) {
  ...
  */
  ...
  kb.ViewModel = (function(_super) {
    ...
    ViewModel.prototype._updateAttributeObservable = function(model, key) {
      ...
      if (this[vm_key]) {
        return this[vm_key].update(model);
      } else {
