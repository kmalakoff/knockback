...
const resolveModule = (module_name) => {
  if (module_name === "underscore")
    return path.relative(".", require.resolve("underscore/underscore-umd.js"));
  return path.relative(".", require.resolve(module_name));
};
...
const REQUIRED_DEPENDENCIES = {
  ...
  backbone_underscore_legacy: [
    "./vendor/underscore-1.1.7.js",
    "./vendor/backbone-0.5.1.js",
    "./vendor/knockout-2.1.0.js",
  ],
  ...
  backbone_lodash_legacy: [
    "./vendor/lodash-0.3.2.js",
    "./vendor/backbone-0.5.1.js",
    "./vendor/knockout-2.1.0.js",
  ],
...
const LOCALIZATION_DEPENCIES = [
  "./test/lib/globalize.js",
...
for (let library_name in KNOCKBACK) {
  ...
  if (
    library_name.indexOf("browser_globals") >= 0 &&
    library_name.indexOf("stack") < 0
  ) {
    for (dep_name in REQUIRED_DEPENDENCIES) {
      dep_files = REQUIRED_DEPENDENCIES[dep_name];
      if (dep_name.indexOf("backbone") >= 0) {
        // Backbone
...
for (test_name in KNOCKBACK) {
  ...
  if (test_name.indexOf("core") >= 0 && test_name.indexOf("stack") < 0) {
    TEST_GROUPS.core.push({
      ...
      files: _.flatten([
        REQUIRED_DEPENDENCIES.backbone_underscore_latest,
        library_files,
        "./test/spec/core/**/*.tests.coffee",
      ]),
    });
...
const ORM_TESTS = {
  backbone_orm: [
    ...
    resolveModule("backbone-orm"),
    "./test/spec/ecosystem/**/backbone-orm*.tests.coffee",
  ],
  backbone_relational: [
    KNOCKBACK.browser_globals,
    resolveModule("backbone-relational"),
    "./test/spec/ecosystem/**/backbone-relational*.tests.coffee",
  ],
  backbone_associations: [
    KNOCKBACK.browser_globals,
    resolveModule("backbone-associations"),
    "./test/spec/ecosystem/**/backbone-associations*.tests.coffee",
  ],
};
...
for (dep_name in object) {
  ...
  for (test_name in ORM_TESTS) {
    test_files = ORM_TESTS[test_name];
    TEST_GROUPS.orm.push({
      name: `${dep_name}_${test_name}`,
      files: _.flatten([dep_files, test_files]),
    });
  }
}
...
TEST_GROUPS.amd = [];
for (let test of Array.from(
  TEST_GROUPS.browser_globals.concat(TEST_GROUPS.core)
)) {
  if (
    test.name.indexOf("_min") < 0 &&
    test.name.indexOf("legacy_") < 0 &&
    test.name.indexOf("parse_") < 0
  ) {
    test_files = test.files.concat([
    ...
    for (file of Array.from(test_files)) {
    ...
    TEST_GROUPS.amd.push({
      name: `amd_${test.name}`,
      files,
      build: {
        files: test_patterns,
        destination: `_temp/amd/${test.name}`,
        options: _.extend({ path_files }, AMD_OPTIONS),
      },
    });
  }
}
...
//##############################
TEST_GROUPS.webpack = [];
for (file of Array.from(FILES.tests_webpack)) {
  TEST_GROUPS.webpack.push({
    name: `webpack_${file.replace(".js", "")}`,
    files: _.flatten([
      file.indexOf("core") >= 0 ? [] : LOCALIZATION_DEPENCIES,
      file,
    ]),
  });
}
...
for (test_name in object1) {
  ...
  TEST_GROUPS.browserify.push({
    ...
    build: {
      destination: test_info.output,
      options: test_info.options,
