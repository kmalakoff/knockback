...
module.exports = {
  libraries: _.flatten(
    _.map(
      _.filter(
        fs.readdirSync("./config/builds/library"),
        (file) =>
          path.extname(file) === ".coffee" &&
          file.indexOf("webpack.config.coffee") >= 0
      ),
      (file) => [
        file.replace("webpack.config.coffee", "js"),
        file.replace("webpack.config.coffee", "min.js"),
      ]
    )
  ),
  ...
  src_core: _.map(
    _.filter(
      fs.readdirSync("./src/core"),
      (file) => path.extname(file) === ".coffee" && file !== "index.coffee"
    ),
    (file) => `./src/core/${file}`
  ),
  ...
  tests_core: (() => {
    const result = [];
    for (filename of Array.from(
      readdirSyncRecursive(__dirname + "/../test/spec/core")
    )) {
  ...
  tests_plugin: (() => {
    const result1 = [];
    for (filename of Array.from(
      readdirSyncRecursive(__dirname + "/../test/spec/plugins")
    )) {
      if (/\.tests.coffee$/.test(filename)) {
  ...
  tests_webpack: _.map(
    _.filter(
      fs.readdirSync("./config/builds/test"),
      (file) =>
        path.extname(file) === ".coffee" &&
        file.indexOf(".tests.webpack.config.coffee") >= 0
    ),
    (file) =>
      `_temp/webpack/${file.replace(
        ".tests.webpack.config.coffee",
        ".tests.js"
      )}`
  ),
};
