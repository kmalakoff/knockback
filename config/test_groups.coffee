...
REQUIRED_DEPENDENCIES =
  ...
  backbone_lodash_legacy: (resolveModule(module_name) for module_name in ['jquery']).concat(['./vendor/optional/lodash-2.4.1.js', './vendor/backbone-0.5.1.js', './vendor/knockout-1.2.1.js'])
  parse_latest: (resolveModule(module_name) for module_name in ['jquery', 'knockout'])
  parse_legacy: (resolveModule(module_name) for module_name in ['jquery', 'knockout']).concat(['./vendor/optional/parse-1.2.0.js'])
...
# ###############################
# ORM_TESTS =
#   backbone_orm: [KNOCKBACK.full, './vendor/optional/backbone-orm-0.6.0.js', './test/spec/ecosystem/**/backbone-orm*.tests.coffee']
