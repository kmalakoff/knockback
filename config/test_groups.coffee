...
for library_name, library_files of KNOCKBACK when (library_name.indexOf('full') >= 0 and library_name.indexOf('stack') < 0)
  for dep_name, dep_files of REQUIRED_DEPENDENCIES
    if dep_name.indexOf('backbone') >= 0 # Backbone
      TEST_GROUPS.full.push({name: "#{dep_name}_#{library_name}", files: _.flatten([dep_files, library_files, MODEL_REF, './test/spec/core/**/*.tests.coffee', './test/spec/plugins/**/*.tests.coffee'])})
    else # Parse
      TEST_GROUPS.full.push({name: "#{dep_name}_#{library_name}", files: _.flatten([dep_files, library_files, './test/spec/core/**/*.tests.coffee', './test/spec/plugins/**/*.tests.coffee'])})
...
#   TEST_GROUPS.webpack.push({name: "webpack_#{file.replace('.js', '')}", files: _.flatten(['./vendor/optional/jquery-2.1.1.js', file])})
...
#   TEST_GROUPS.browserify.push({name: "browserify_#{test_name}", files: _.flatten(['./vendor/optional/jquery-2.1.1.js', test_info.output]), build: {destination: test_info.output, options: test_info.options, files: test_info.files}})
