...
module.exports = function (options, callback) {
  ...
  for (let name in TEST_GROUPS) {
    const tests = TEST_GROUPS[name];
    ((name, tests) =>
      Array.from(tests).map((test) =>
        ((
          test // return unless test.name is 'amd_backbone_lodash_latest_browser_globals'
        ) =>
          queue.defer(function (callback) {
            gutil.log(`RUNNING TESTS: ${name} ${test.name}`);
            gutil.log(`${JSON.stringify(test.files)}`);
            const karma_config =
              name === "amd" ? KARMA_CONFIG_AMD : KARMA_CONFIG_BASE;
            const args = options.tags ? ["--grep", options.tags] : [];
            return new Server(
              _.defaults({ files: test.files, client: { args } }, karma_config),
              function (return_value) {
                console.log(
                  `DONE TESTS: ${name} ${test.name}. Return value: ${return_value}`
                );
                return callback(
                  return_value
                    ? new Error(`Tests failed: ${return_value}`)
                    : undefined
                );
              }
            ).start();
