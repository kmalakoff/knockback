...
let CollectionObservable = class CollectionObservable {
    ...
    viewModelByModel(model) {
        ...
        return _.find(kb.peek(kb.utils.wrappedObservable(this)), function(test) {
            if (__guard__(test != null ? test.__kb : undefined, (x)=>x.object)) {
    ...
    _onObservableArrayChange(models_or_view_models) {
        return kb.ignore(()=>{
            ...
            // set ViewModels
                ...
                models = [];
                for (var view_model of Array.from(models_or_view_models)){
    ...
    _selectModel(model) {
        ...
        for (var filter of Array.from(filters)){
    ...
    constructor(collection, _view_model, options){
        ...
        return kb.ignore(()=>{
            ...
            this._mapper = ko.computed(()=>{
                ...
                if (filters) {
                    for (filter of Array.from(filters)){
                ...
                const _previous_view_models = kb.peek(observable);
...
//
export { CollectionObservable as default };
// factory function
