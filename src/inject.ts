...
/**
 * Inject class for DOM-based view model injection
 */
export class Inject {
  /**
   ...
   */
  static inject(
    data: InjectData | (new (...args: unknown[]) => unknown),
    viewModel: Record<string, unknown>,
    element: Element,
    valueAccessor?: unknown,
    allBindingsAccessor?: unknown,
    nested?: boolean
  ): unknown {
      ...
      if (typeof injectData === 'function') {
        ...
        for (const key in injectData) {
          ...
          // Resolve nested with assign or not
            ...
            viewModel[key] = Inject.inject(nestedData, target, element, valueAccessor, allBindingsAccessor, true);
          }
          // Simple set
          else {
            viewModel[key] = value;
   ...
   */
  static injectViewModels(root?: Element | Document): InjectResult[] {
    ...
    for (const app of results) {
      ...
      if (app.binding) {
        ...
        try {
          ...
          app.view_model = Inject.inject(data, app.view_model as Record<string, unknown>, app.el, null, null, true);
...
/**
 * Alias for Inject.injectViewModels
 */
export const injectViewModels = Inject.injectViewModels;
  ...
  if (ko.bindingHandlers) {
    ko.bindingHandlers['inject'] = {
      init: (element: Element, valueAccessor: () => unknown, allBindingsAccessor: ko.AllBindings, viewModel: unknown) => {
        Inject.inject(ko.utils.unwrapObservable(valueAccessor()) as InjectData, viewModel as Record<string, unknown>, element, valueAccessor, allBindingsAccessor);
