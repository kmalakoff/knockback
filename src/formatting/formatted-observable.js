...
kb.parseFormattedString = function (string, format) {
  ...
  const sorted_positions = _.sortBy(_.keys(positions), (parameter_index, _format_index) => parseInt(parameter_index, 10));
    ...
    const observable = kb.utils.wrappedObservable(
      ...
      ko.computed({
        read() {
          ...
          for (var arg of Array.from(observable_args)) {
        ...
        write(value) {
          ...
          const max_count = Math.min(observable_args.length, matches.length);
          let index = 0;
