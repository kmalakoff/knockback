...
module.exports = kb = (function () {
  kb = class kb {
    static initClass() {
      ...
      //   kb.ignore(fn);
      kb.ignore =
        (ko.dependencyDetection != null ? ko.dependencyDetection.ignore : undefined) ||
    ...
    static isReleaseable(obj, depth) {
      ...
      for (var method of Array.from(LIFECYCLE_METHODS)) {
    ...
    static renderTemplate(template, view_model, options) {
      ...
      if (el.childNodes.length === 1) {
        ...
        for (let i = 0, end = el.childNodes.length, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
    ...
    static getValue(model, key, args) {
      ...
      return model.get.apply(
        model,
        _.map([key].concat(args), (value) => kb.peek(value))
      );
    }
    ...
    static publishMethods(observable, instance, methods) {
      for (var fn of Array.from(methods)) {
