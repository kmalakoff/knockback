...
module.exports = TypedValue = class TypedValue {
  ...
  destroy() {
    ...
    if (previous_value = this.__kb_value) {
      ...
      if (this.create_options.store && kb.utils.wrappedCreator(previous_value)) { this.create_options.store.release(previous_value); } else { kb.release(previous_value); }
    }
  ...
  update(new_value) {
    ...
    if (this.__kb_value != null ? this.__kb_value.__kb_released : undefined) { (this.__kb_value = (this.value_type = undefined)); }
      ...
      case kb.TYPE_COLLECTION:
        ...
        if ((new_type === kb.TYPE_COLLECTION) || _.isNull(new_value)) {
          ...
          if (new_value && new_value instanceof kb.CollectionObservable) {
            ...
            if (kb.peek(value.collection) !== new_value) { value.collection(new_value); } // collection observables are allocated once
          }
          return;
        }
        break;
  ...
  _updateValueObservable(new_value, new_observable) {
    ...
    if ((this.value_type = value_type) === kb.TYPE_UNKNOWN) {
      if (!ko.isObservable(value)) { // a view model, recognize view_models as non-observable
    ...
    // release previous
    if (previous_value) {
      if (this.create_options.store) { this.create_options.store.release(previous_value); } else { kb.release(previous_value); }
    }
