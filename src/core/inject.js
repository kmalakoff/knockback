...
kb.Inject = class Inject {
  ...
  static injectViewModels(root) {
    ...
    var findElements = function (el) {
      ...
      for (var child_el of Array.from(el.childNodes)) {
    ...
    for (var app of Array.from(results)) {
      ...
      if ((expression = app.binding)) {
        ...
        data || (data = {}); // no data
        !data.options || (({ options } = data), (data.options = undefined)); // extract options
        options || (options = {});
