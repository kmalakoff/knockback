  ...
  */
  ...
  kb.Factory = (function() {
    ...
    Factory.prototype.createForPath = function(obj, path, store) {
      ...
      if (create_info.view_model) {
        return new create_info.view_model(obj, options);
      }
      if (create_info.create) {
        return create_info.create(obj, options);
      }
      throw Error('unrecognized create function');
    };
  ...
  */
  ...
  kb.CollectionObservable = (function(_super) {
    ...
    function CollectionObservable(collection, options) {
      ...
      this.factory = new kb.Factory(options.path, options.factory);
      if (options.mappings) {
        this.factory.addPathMappings(options.mappings);
      }
      if (options.view_model) {
        this.factory.addPathMapping('models', options.view_model);
      } else if (options.create) {
        this.factory.addPathMapping('models', {
    ...
    CollectionObservable.prototype._createTarget = function(model) {
      if (this.hasViewModels()) {
        return this.__kb.store.resolveValue(model, this.factory, 'models');
      } else {
        return model;
      }
    };
  ...
  */
  ...
  kb.ViewModel = (function(_super) {
    ...
    ViewModel.prototype._updateAttributeObservable = function(model, key) {
      var observable, value, vm_key;
      vm_key = this.__kb.internals && _.contains(this.__kb.internals, key) ? '_' + key : key;
      value = model.get(key);
      observable = this[vm_key];
      if (value instanceof Backbone.Model) {
        if (observable) {
          if (kb.utils.wrappedModel(observable) !== value) {
            if (_.isFunction(observable.model)) {
              observable.model(value);
            }
            return kb.utils.wrappedModel(observable, value);
          }
        } else {
          observable = this.__kb.store.resolveValue(value, this.factory, key);
          return this[vm_key] = ko.isObservable(observable) ? observable : ko.observable(observable);
        }
      } else if (value instanceof Backbone.Collection) {
        if (observable) {
          if (observable.collection() !== value) {
            return observable.collection(value);
          }
        } else {
          return this[vm_key] = observable = this.__kb.store.resolveValue(value, this.factory, key);
        }
      } else {
        if (observable) {
          if (observable() !== value) {
          ...
          return this[vm_key] = ko.observable(value);
        }
      }
    };
