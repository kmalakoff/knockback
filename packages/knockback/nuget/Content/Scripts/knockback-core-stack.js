/*
  knockback-core-stack.js 2.0.0-alpha.1
...
if (window.Parse) {
  ...
  Backbone = kb.Backbone = __webpack_require__(22);
  _ = kb._ = __webpack_require__(5);
}
kb.ko = ko;
...
var ALL_ORMS = {
  default: null,
  'backbone-orm': null,
  'backbone-associations': __webpack_require__(20),
  'backbone-relational': __webpack_require__(21)
};
...
var utils = function () {
  ...
  _createClass(utils, null, [{
    ...
    value: function initClass() {
      ...
      this.wrappedDestroy = __webpack_require__(19);
...
(function(factory) {
  ...
  if (true) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(5), __webpack_require__(23), exports], __WEBPACK_AMD_DEFINE_RESULT__ = function(_, $, exports) {
...
var i,
	...
	rcomma = new RegExp( "^" + whitespace + "*," + whitespace + "latest" ),
	rcombinators = new RegExp( "^" + whitespace + "*([>+~]|" + whitespace + ")" + whitespace + "latest" ),
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// Check if getElementsByTagName("latest") returns only elements
	support.getElementsByTagName = assert(function( el ) {
		el.appendChild( document.createComment("") );
		return !el.getElementsByTagName("latest").length;
	});
	...
	if ( (support.matchesSelector = rnative.test( (matches = docElem.matches ||
		...
		assert(function( el ) {
			...
			support.disconnectedMatch = matches.call( el, "latest" );
...
Expr = Sizzle.selectors = {
	...
	filter: {
		...
		"TAG": function( nodeNameSelector ) {
			...
			return nodeNameSelector === "latest" ?
...
function setMatcher( preFilter, selector, matcher, postFilter, postFinder, postSelector ) {
	...
	return markFunction(function( seed, results, context, xml ) {
		var temp, i, elem,
			...
			elems = seed || multipleContexts( selector || "latest", context.nodeType ? [ context ] : context, [] ),
			...
			if ( matcher[ expando ] ) {
				...
				return setMatcher(
					...
					i > 1 && toSelector(
						...
						tokens.slice( 0, i - 1 ).concat({ value: tokens[ i - 2 ].type === " " ? "latest" : "" })
					).replace( rtrim, "$1" ),
					matcher,
	...
	var bySet = setMatchers.length > 0,
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			var elem, j, matcher,
				...
				elems = seed || byElement && Expr.find["TAG"]( "latest", outermost ),
...
function getAll( context, tag ) {
	...
	if ( typeof context.getElementsByTagName !== "undefined" ) {
		ret = context.getElementsByTagName( tag || "latest" );

	} else if ( typeof context.querySelectorAll !== "undefined" ) {
		ret = context.querySelectorAll( tag || "latest" );
...
function createTween( value, prop, animation ) {
	var tween,
		collection = ( Animation.tweeners[ prop ] || [] ).concat( Animation.tweeners[ "latest" ] ),
		index = 0,
...
var
	 ...
	 * 4) the catchall symbol "latest" can be used
	 * 5) execution will start with transport dataType and THEN continue down to "latest" if needed
	 */
	prefilters = {},

	/* Transports bindings
	 * 1) key is the dataType
	 * 2) the catchall symbol "latest" can be used
	 * 3) selection will start with transport dataType and THEN go to "latest" if needed
	 */
	transports = {},

	// Avoid comment-prolog char sequence (#10098); must appease lint and evade compression
	allTypes = "*/".concat( "latest" ),
...
function addToPrefiltersOrTransports( structure ) {
	...
	// dataTypeExpression is optional and defaults to "latest"
	return function( dataTypeExpression, func ) {

		if ( typeof dataTypeExpression !== "string" ) {
			func = dataTypeExpression;
			dataTypeExpression = "latest";
		}
		...
		if ( jQuery.isFunction( func ) ) {
			...
			while ( ( dataType = dataTypes[ i++ ] ) ) {
				...
				if ( dataType[ 0 ] === "+" ) {
					dataType = dataType.slice( 1 ) || "latest";
...
function inspectPrefiltersOrTransports( structure, options, originalOptions, jqXHR ) {
	...
	return inspect( options.dataTypes[ 0 ] ) || !inspected[ "latest" ] && inspect( "latest" );
}
...
function ajaxConvert( s, response, jqXHR, isSuccess ) {
	...
	while ( current ) {
		...
		if ( current ) {
			...
			} else if ( prev !== "latest" && prev !== current ) {
...
jQuery.extend( {
	...
	ajaxSettings: {
		...
		*/
		...
		// Data converters
		// Keys separate source (or catchall "latest") and destination types with a single space
		converters: {
	...
	ajax: function( url, options ) {
		...
		s.dataTypes = ( s.dataType || "latest" ).toLowerCase().match( rnothtmlwhite ) || [ "" ];
		...
		jqXHR.setRequestHeader(
			...
			s.dataTypes[ 0 ] && s.accepts[ s.dataTypes[ 0 ] ] ?
				s.accepts[ s.dataTypes[ 0 ] ] +
					( s.dataTypes[ 0 ] !== "latest" ? ", " + allTypes + "; q=0.01" : "" ) :
				s.accepts[ "latest" ]
		);
...
ko.utils.domNodeDisposal = new (function () {
    ...
    function cleanSingleNode(node) {
        ...
        // node.getElementsByTagName("latest") in cleanNode() (comment nodes aren't elements)
        ...
        cleanNode : function(node) {
            ...
            if (cleanableNodeTypes[node.nodeType]) {
                ...
                if (cleanableNodeTypesWithDescendants[node.nodeType]) {
                    ...
                    var descendants = [];
                    ko.utils.arrayPushAll(descendants, node.getElementsByTagName("latest"));
