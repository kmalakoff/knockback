...
(function (window, document, navigator, undefined) {
  !(function (factory) {
    ...
    kb = (function () {
      ...
      kb.release = function (obj, pre_release_fn) {
        ...
        if (
          !obj ||
          obj !== Object(obj) ||
          (typeof obj === "function" && !ko.isObservable(obj)) ||
          obj.__kb_destroyed ||
        ...
        if (
          ...
          if (ko.isObservable(obj) && _.isArray((array = obj()))) {
            if (
              obj.__kb_is_co ||
              (obj.__kb_is_o && obj.valueType() === KB_TYPE_COLLECTION)
            ) {
              if (obj.destroy) {
                obj.destroy();
              } else if (obj.dispose) {
    ...
    kb.Backbone = Backbone =
      !this.Backbone && typeof require !== "undefined"
        ? require("backbone")
        : this.Backbone;

    kb.ko = ko =
      !this.ko && typeof require !== "undefined"
        ? require("knockout")
        : this.ko;
    ...
    legacyWarning = function (identifier, last_version, message) {
      ...
      return console.warn(
        "warning: '" +
          identifier +
          "' has been deprecated (will be removed in Knockback after " +
          last_version +
          "). " +
          message +
          "."
  ...
  */
    ...
    kb.utils = (function () {
      ...
      utils.wrappedObservable = function (obj, value) {
        return _wrappedKey.apply(
          this,
          _argumentsAddKey(arguments, "observable")
        );
      };
      ...
      utils.wrappedStoreIsOwned = function (obj, value) {
        return _wrappedKey.apply(
          this,
          _argumentsAddKey(arguments, "store_is_owned")
        );
      };
      ...
      utils.wrappedModelWatcher = function (obj, value) {
        return _wrappedKey.apply(
          this,
          _argumentsAddKey(arguments, "model_watcher")
        );
      };
      ...
      utils.wrappedModelWatcherIsOwned = function (obj, value) {
        return _wrappedKey.apply(
          this,
          _argumentsAddKey(arguments, "model_watcher_is_owned")
        );
      };
      ...
      utils.valueType = function (observable) {
        ...
        if (
          ...
          observable instanceof Backbone.Collection
        ) {
          return KB_TYPE_COLLECTION;
        }
        if (
          observable instanceof kb.ViewModel ||
          observable instanceof Backbone.Model
        ) {
      ...
      utils.inferCreator = function (value, factory, path, owner, key) {
        ...
        if (
          ...
          owner instanceof Backbone.RelationalModel
        ) {
    ...
    kb.Factory = (function () {
      ...
      Factory.prototype.hasPath = function (path) {
        return (
          ...
          (this.parent_factory && this.parent_factory.hasPath(path))
        );
      };
    ...
    kb.Store = (function () {
      ...
      Store.prototype.register = function (obj, observable, options) {
        ...
        creator = options.creator
          ? options.creator
          : options.path && options.factory
          ? options.factory.creatorForPath(obj, options.path)
          : null;
        if (!creator) {
      ...
      Store.prototype.findIndex = function (obj, creator) {
        ...
        if (!obj || obj instanceof Backbone.Model) {
          ...
          for (index in _ref) {
            ...
            if (
              (!obj && !record.observable.__kb_null) ||
              (obj && (record.observable.__kb_null || record.obj !== obj))
            ) {
              continue;
            } else if (
              record.creator === creator ||
              (record.creator.create &&
                record.creator.create === creator.create)
            ) {
              return index;
            }
          }
      ...
      Store.prototype.findOrCreate = function (obj, options) {
        ...
        options.store = this;
        options.creator ||
          (options.creator = kb.utils.inferCreator(
            obj,
            options.factory,
            options.path
          ));
      ...
      Store.prototype.findOrReplace = function (obj, creator, observable) {
        ...
        if ((index = this.findIndex(obj, creator)) < 0) {
          ...
          kb.utils.wrappedObject(record.observable) === obj ||
            throwUnexpected(this, "different object");
          if (record.observable !== observable) {
            record.observable.constructor === observable.constructor ||
              throwUnexpected(this, "replacing different type");
  ...
  */
    ...
    kb.ModelWatcher = (function () {
      ModelWatcher.useOptionsOrCreate = function (
        options,
        model,
        obj,
        callback_options
        ...
        if (options.model_watcher) {
          if (
            !(
              options.model_watcher.model() === model ||
              options.model_watcher.model_ref === model
            )
          ) {
            throwUnexpected(this, "model not matching");
          }
          return kb.utils
            .wrappedModelWatcher(obj, options.model_watcher)
            .registerCallbacks(obj, callback_options);
        } else {
          kb.utils.wrappedModelWatcherIsOwned(obj, true);
          return kb.utils
            .wrappedModelWatcher(obj, new kb.ModelWatcher(model))
            .registerCallbacks(obj, callback_options);
        }
      };
      ...
      ModelWatcher.prototype.registerCallbacks = function (obj, callback_info) {
              ...
              for (_i = 0, _len = list.length; _i < _len; _i++) {
                ...
                if (info.update && !info.rel_fn) {
                  if (
                    ...
                    info.key &&
                    model.hasChanged &&
                    !model.hasChanged(ko.utils.unwrapObservable(info.key))
                  ) {
                    continue;
                  }
        ...
        if (this.m) {
          if (
            Backbone.RelationalModel &&
            this.m instanceof Backbone.RelationalModel
          ) {
      ...
      ModelWatcher.prototype._onModelLoaded = function (model) {
        ...
        is_relational =
          Backbone.RelationalModel && model instanceof Backbone.RelationalModel;
        this.m = model;
        _ref = this.__kb.callbacks;
        for (event_name in _ref) {
          callbacks = _ref[event_name];
      ...
      ModelWatcher.prototype._modelBindRelatationalInfo = function (
        event_name,
        info
      ) {
        var key, relation;
      ...
      ModelWatcher.prototype._modelUnbindRelatationalInfo = function (
        event_name,
        info
      ) {
        if (!info.rel_fn) {
  ...
  */
    ...
    kb.Observable = (function () {
      function Observable(model, options, vm) {
        ...
        !create_options.args ||
          ((this.args = create_options.args), delete create_options.args);
        !create_options.read ||
          ((this.read = create_options.read), delete create_options.read);
        !create_options.write ||
          ((this.write = create_options.write), delete create_options.write);
        ...
        observable = kb.utils.wrappedObservable(
          ...
          ko.dependentObservable({
            read: function () {
              ...
              if (_this.m) {
                new_value = _this.read
                  ? _this.read.apply(_this.vm, args)
                  : _this.m.get.apply(_this.m, args);
            ...
            write: function (new_value) {
              ...
              set_info = {};
              set_info[ko.utils.unwrapObservable(_this.key)] =
                unwrapped_new_value;
      ...
      Observable.prototype.update = function (new_value) {
        ...
        if (
          !this.__kb_value ||
          this.__kb_value.__kb_destroyed ||
          (this.__kb_value.__kb_null && new_value)
        ) {
          this.__kb_value = null;
        ...
        if (
          ...
          (this.value_type !== new_type && new_type !== KB_TYPE_UNKNOWN)
        ) {
          if (
            this.value_type === KB_TYPE_COLLECTION &&
            new_type === KB_TYPE_ARRAY
          ) {
      ...
      Observable.prototype._updateValueObservable = function (new_value) {
        ...
        create_options = this.create_options;
        create_options.creator = kb.utils.inferCreator(
          new_value,
        ...
        if (creator) {
          if (create_options.store) {
            value = create_options.store.findOrCreate(
              new_value,
              create_options
            );
          } else {
  ...
  */
    ...
    kb.ViewModel = (function () {
      ...
      function ViewModel(model, options, view_model) {
        ...
        !model ||
          ...
          (typeof model.get === "function" &&
            typeof model.bind === "function") ||
          throwUnexpected(this, "not a model");
        options || (options = {});
        ...
        model_watcher = kb.utils.wrappedModelWatcher(
          this,
          new kb.ModelWatcher(model, this, {
            model: _.bind(this.model, this),
          })
        );
        ...
        if (this.__kb.internals) {
          keys = keys
            ? _.union(keys, this.__kb.internals)
            : _.clone(this.__kb.internals);
        }
        if (options.keys) {
      ...
      ViewModel.prototype.model = function (new_model) {
        ...
        missing = _.difference(
          _.keys(new_model.attributes),
          _.keys(this.__kb.model_keys)
        );
        if (missing) {
      ...
      ViewModel.prototype._createObservables = function (model, keys) {
        ...
        for (_i = 0, _len = keys.length; _i < _len; _i++) {
          key = keys[_i];
          vm_key =
            this.__kb.internals && ~_.indexOf(this.__kb.internals, key)
              ? "_" + key
              : key;
          if (this[vm_key]) {
            continue;
          ...
          this[vm_key] = this.__kb.view_model[vm_key] = kb.observable(
            model,
            create_options,
            this
          );
      ...
      ViewModel.prototype._mapObservables = function (model, mappings) {
        ...
        for (vm_key in mappings) {
          ...
          this[vm_key] = this.__kb.view_model[vm_key] = kb.observable(
            model,
            _.defaults(mapping_info, create_options),
            this
          );
        }
        return this;
    ...
    kb.observables = function (model, binding_info, view_model) {
      legacyWarning(
        "ko.observables",
        "0.16.2",
        "Please use kb.viewModel instead"
      );
  ...
  */
    ...
    kb.CollectionObservable = (function () {
      ...
      function CollectionObservable(collection, options) {
        ...
        !collection ||
          collection instanceof Backbone.Collection ||
          throwUnexpected(this, "not a collection");
        options || (options = {});
        ...
        this.filters = _.isArray(options.filters)
          ? options.filters
          : options.filters
          ? [options.filters]
          : void 0;
        ...
        factory = create_options.factory = kb.utils.wrappedFactory(
          observable,
          new kb.Factory(options.factory)
        );
        if (options.factories) {
        ...
        create_options.creator = factory.creatorForPath(
          null,
          create_options.path
        );
      ...
      CollectionObservable.prototype.sortAttribute = function (
        sort_attribute,
        sorted_index,
        options
      ) {
      ...
      CollectionObservable.prototype._onCollectionChange = function (
        event,
        arg
      ) {
        ...
        switch (event) {
          ...
          case "add":
            ...
            add_index = this.sorted_index
              ? this.sorted_index(observable(), view_model)
      ...
      CollectionObservable.prototype._onModelChange = function (model) {
        if (this._modelIsFiltered(model)) {
          ...
          if (
            this.sorted_index &&
            (!this.sort_attribute || model.hasChanged(this.sort_attribute))
          ) {
      ...
      CollectionObservable.prototype._onObservableArrayChange = function (
        values
      ) {
        var collection,
          has_view_model,
        ...
        if (!this.models_only) {
          ...
          if (has_view_model) {
            for (_j = 0, _len1 = values.length; _j < _len1; _j++) {
              value = values[_j];
              this.create_options.store.findOrReplace(
                kb.utils.wrappedObject(value),
                this.create_options.creator,
                value
              );
            }
          }
        ...
        if (
          models.length !== values.length ||
          _.difference(collection.models, models).length
        ) {
          this.in_edit++;
      ...
      CollectionObservable.prototype._sortAttributeFn = function (
        sort_attribute
      ) {
        if (this.models_only) {
      ...
      CollectionObservable.prototype._createViewModel = function (model) {
        if (this.models_only) {
          ...
          return this.create_options.store.findOrCreate(
            model,
            this.create_options
          );
        }
      ...
      CollectionObservable.prototype._modelIsFiltered = function (model) {
        ...
        if (this.filters) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if (
              (typeof filter === "function" && filter(model)) ||
    ...
    kb.sortedIndexWrapAttr = kb.siwa = function (
      attribute_name,
      wrapper_constructor
    ) {
      return function (models, model) {
        return _.sortedIndex(models, model, function (test) {
          return new wrapper_constructor(
            kb.utils.wrappedModel(test).get(attribute_name)
          );
        });
      };
    };
  ...
  */
    ...
    kb.defaultWrapper = function (target, default_value) {
      legacyWarning(
        "ko.defaultWrapper",
        "0.16.2",
        "Please use kb.defaultObservable instead"
      );
      return new kb.DefaultObservable(target, default_value);
    };
