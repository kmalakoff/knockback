...
(function (window, document, navigator, undefined) {
  !(function (factory) {
    ...
    kb = (function () {
      ...
      kb.releaseKeys = function (obj) {
        ...
        for (key in obj) {
          value = obj[key];
          (key === '__kb') || kb.release(value, (() => obj[key] = null));
        }
        return this;
      };
  ...
  */
    ...
    _unwrapModels = function (obj) {
      ...
      if (!obj) {
        ...
        return _.map(obj, test => _unwrapModels(test));
    ...
    kb.ModelWatcher = (function () {
      ...
      ModelWatcher.prototype._modelBindRelatationalInfo = function (event_name, info) {
        ...
        if ((event_name === 'change') && info.key && info.update) {
          ...
          relation = _.find(this.m.getRelations(), test => test.key === key);
          if (!relation) {
            return;
          }
  ...
  */
    ...
    kb.CollectionObservable = (function () {
      ...
      CollectionObservable.prototype.viewModelByModel = function (model) {
        ...
        return _.find(kb.utils.wrappedObservable(this)(), test => test.__kb.object[id_attribute] === model[id_attribute]);
      };
      ...
      CollectionObservable.prototype._onObservableArrayChange = function (values) {
        ...
        models = _.map(values, test => kb.utils.wrappedModel(test));
        if (this.filters) {
          models = _.filter(models, model => !_this._modelIsFiltered(model));
        }
      ...
      CollectionObservable.prototype._collectionResync = function (silent) {
        ...
        array.splice(0, array.length);
        if (this.filters) {
          models = _.filter(collection.models, model => !_this._modelIsFiltered(model));
        } else {
        ...
        if (this.sorted_index) {
          ...
          view_models = this.models_only ? (this.filters ? models : _.clone(models)) : _.map(models, model => _this._createViewModel(model));
        }
        this.in_edit++;
      ...
      CollectionObservable.prototype._sortAttributeFn = function (sort_attribute) {
        if (this.models_only) {
          return function (models, model) {
            return _.sortedIndex(models, model, test => test.get(sort_attribute));
          };
        }
        return function (view_models, model) {
          return _.sortedIndex(view_models, model, test => kb.utils.wrappedModel(test).get(sort_attribute));
        };
      };
    ...
    kb.sortedIndexWrapAttr = kb.siwa = function (attribute_name, wrapper_constructor) {
      return function (models, model) {
        return _.sortedIndex(models, model, test => new wrapper_constructor(kb.utils.wrappedModel(test).get(attribute_name)));
      };
    };
  ...
  */
    ...
    kb.parseFormattedString = function (string, format) {
      ...
      sorted_positions = _.sortBy(_.keys(positions), (parameter_index, format_index) => parseInt(parameter_index, 10));
      format_indices_to_matched_indices = {};
