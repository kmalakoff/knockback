...
(function (global) {
  ...
  function newContext(contextName) {
    ...
    function hasPathFallback(id) {
      const pathConfig = config.paths[id];
    ...
    Module.prototype = {
      ...
      defineDepById(id, depExports) {
        let i;

                // Find the index for this dependency.
        each(this.depMaps, (map, index) => {
          if (map.id === id) {
            i = index;
            return true;
          }
      ...
      callPlugin() {
        ...
        on(pluginMap, 'defined', bind(this, function (plugin) {
          ...
          plugin.load(map.name, makeRequire(map.parentMap, true, (deps, cb, er) => {
      ...
      enable() {
        ...
        eachProp(this.pluginMaps, bind(this, function (pluginMap) {
          const mod = registry[pluginMap.id];
          if (mod && !mod.enabled) {
            context.enable(pluginMap, this);
          }
        }));
    ...
    return (context = {
      ...
      requireDefined(id, relMap) {
        return hasProp(defined, makeModuleMap(id, relMap, false, true).id);
      },

      requireSpecified(id, relMap) {
        id = makeModuleMap(id, relMap, false, true).id;
        return hasProp(defined, id) || hasProp(registry, id);
      },

      require(deps, callback, errback, relMap) {
        let moduleName,
          id,
          map,
          requireMod,
      ...
      undef(id) {
        ...
        let map = makeModuleMap(id, null, true),
          mod = registry[id];
      ...
      completeLoad(moduleName) {
        ...
        checkLoaded();
             ...
             */
      toUrl(moduleNamePlusExt, relModuleMap) {
        let index = moduleNamePlusExt.lastIndexOf('.'),
          ext = null;
