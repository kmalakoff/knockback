try
  require.config({
    paths:
      ...
      'knockback-formatting': "../../../../lib/formatting",
  ...
  require ['underscore', 'backbone', 'knockout', module_name, 'knockback-formatting', 'knockback-statistics', 'mocha_test_runner'], (_, Backbone, ko, kb, kbf, kbs, runner) ->
    window._ = window.Backbone = window.ko = window.kb = null # force each test to require dependencies synchronously
    require ['../../../knockback/formatting/build/test'], -> runner.start()
