$(document).ready( ->
  ...
  test("Collection sync dynamically changing the sorting function", ->
    ...
    collection_observable.sortedIndex(((view_models, vm) -> _.sortedIndex(view_models, vm, (test) -> kb.vmModel(test).get('name'))), 'name')
    equal(collection.models[0].get('name'), 'Ringo', "Ringo is first")
    equal(collection.models[1].get('name'), 'George', "George is second")
    equal(kb.vmModel(collection_observable()[0]).get('name'), 'George', "George is first - sorting worked!")
    equal(kb.vmModel(collection_observable()[1]).get('name'), 'Ringo', "Ringo is second - sorting worked!")
  ...
  test("Error cases", ->
    ...
    raises((->kb.collectionObservable(new ContactsCollection(), ko.observableArray([]))), Error, 'CollectionObservable: external ko.observableArray are no longer supported as of Knockback V0.15.0. Please use the kb.collectionObservable directly instead')
