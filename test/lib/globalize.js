...
//
  ...
  formatDate = function (value, format, culture) {
    ...
    if (!format || !format.length || format === 'i') {
      ...
      if (culture && culture.name.length) {
        if (convert) {
          ...
          let eraDate = new Date(value.getTime()),
    ...
    for (; ;) {
      ...
      let index = tokenRegExp.lastIndex,
...
// parseExact
  (function () {
    ...
    getMonthIndex = function (cal, value, abbr) {
      ...
      value = toUpper(value);
      let i = arrayIndexOf(abbr ? upperMonths[1] : upperMonths[0], value);
      if (i < 0) {
    ...
    getParseRegExp = function (cal, format) {
      ...
      let expFormat = expandFormat(cal, format).replace(/([\^\$\.\*\+\?\|\[\]\(\)\{\}])/g, '\\\\$1'),
        regexp = ['^'],
        groups = [],
        index = 0,
        quoteCount = 0,
      ...
      let regexpStr = regexp.join('').replace(/\s+/g, '\\s+'),
        parseRegExp = { regExp: regexpStr, groups };
...
//
  ...
  Globalize.findClosestCulture = function (name) {
    ...
    if (isArray(name)) {
      ...
      prioritized.sort((a, b) => a.pri < b.pri ? 1 : -1);
      ...
      for (i = 0; i < l; i++) {
        lang = prioritized[i].lang;
        for (const cultureKey in cultures) {
  ...
  Globalize.parseFloat = function (value, radix, cultureSelector) {
    ...
    const culture = this.findClosestCulture(cultureSelector);
    let ret = NaN,
    ...
    if (regexInfinity.test(value)) {
      ...
      let signInfo = parseNegativePattern(value, nf, nf.pattern[0]),
        sign = signInfo[0],
        num = signInfo[1];
      ...
      const altGroupSep = groupSep.replace(/\u00A0/g, ' ');
      ...
      if (exponent !== null) {
        ...
        const expSignInfo = parseNegativePattern(exponent, nf, '-n');
