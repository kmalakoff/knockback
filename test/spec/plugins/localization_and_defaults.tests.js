...
const { _, ko } = kb;
if (kb.Backbone) {
  kb.Backbone.ModelRef || (typeof require === 'function' ? require('backbone-modelref') : undefined);
}
...
class LocaleManager {
  ...
  getLocales() {
    ...
    for (var string_id in this.translations_by_locale) {
      var _value = this.translations_by_locale[string_id];
...
class LocalizedStringLocalizer extends kb.LocalizedObservable {
  constructor(_value, _options, _view_model) {
    super(...arguments);
...
describe('localized-observable @quick @localization', function () {
  ...
  it('2. internals test (Coffeescript inheritance)', function (done) {
    ...
    if (typeof process !== 'undefined' && process.browser) {
      return done();
    } // TODO: skip on AMD until have time to troubleshoot
...
describe('defaults @quick @defaults', function () {
  it('TEST DEPENDENCY MISSING', function (done) {
    assert.ok(!!ko, 'ko');
    assert.ok(!!_, '_');
    assert.ok(!!kb.Model, 'kb.Model');
  ...
  it.skip('3. internals test (Coffeescript inheritance)', function (done) {
    // TODO: fix these tests in browserify
    if (typeof process !== 'undefined' && process.browser) {
      return done();
    } // TODO: skip on AMD until have time to troubleshoot
  ...
  return it('4. 0 should not behave as null', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats

    const model = new Contact();
    const casebutton = kb.observable(model, { key: 'casebutton', default: 99 });
    const casecond = ko.computed(function () {
      switch (casebutton()) {
        case 0:
          return 'Open';
        case 1:
