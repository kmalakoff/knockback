...
describe('Knockback.js with BackboneORM @backbone-orm', function () {
  ...
  it('TEST DEPENDENCY MISSING', function (done) {
    ...
    kb.configure({ orm: 'backbone-orm' });
    kb.configure({ orm: 'default' });
    return done();
  });
  ...
  class Person extends Backbone.Model {
    static initClass() {
      ...
      Person.prototype.schema = {
        id: [{ manual: true }],
        friends() {
          return ['hasMany', Person, { foreign_key: 'friends_id', as: 'friends_with_me' }];
        },
        friends_with_me() {
          return ['hasMany', Person, { foreign_key: 'friends_with_me_id', as: 'friends' }];
        },
        best_friend() {
          return ['belongsTo', Person, { as: 'best_friends_with_me' }];
        },
        best_friends_with_me() {
          return ['hasMany', Person, { as: 'best_friend' }];
        },
        occupies() {
          return ['belongsTo', Building, { as: 'occupants' }];
        },
      };
  ...
  class Building extends Backbone.Model {
    static initClass() {
      ...
      Building.prototype.schema = {
        id: [{ manual: true }],
        occupants() {
          return ['hasMany', Person, { as: 'occupies' }];
        },
      };
  ...
  it('1. Model with HasMany relations: A house with multiple people living in it', function (done) {
    ...
    for (var occupant_observable of Array.from(house_view_model.occupants())) {
      ...
      for (var occupant_observable2 of Array.from(occupant_observable.occupies().occupants())) {
  ...
  it('2. Collection with models with HasMany relations: Multiple houses with multiple people living in them', function (done) {
    ...
    let occupant_observable2;
    let place_view_model;
    kb.statistics = new kb.Statistics(); // turn on stats
    ...
    for (place_view_model of Array.from(places_observable())) {
      if (place_view_model.id() === 'house-2-1') {
        ...
        for (occupant_observable of Array.from(place_view_model.occupants())) {
          ...
          for (occupant_observable2 of Array.from(occupant_observable.occupies().occupants())) {
  ...
  it('3. Model with recursive HasMany relations: Person with users who are people', function (done) {
    ...
    return queue.await(function (err) {
      ...
      for (friend of Array.from(john_view_model.friends())) {
      ...
      for (friend of Array.from(paul_view_model.friends())) {
      ...
      for (friend of Array.from(george_view_model.friends())) {
  ...
  it('4. After view model create, add models', function (done) {
    class Occupant extends Backbone.Model {
      static initClass() {
        ...
        Occupant.prototype.schema = {
          livesIn() {
            return ['belongsTo', House, { as: 'occupants' }];
          },
        };
    ...
    class House extends Backbone.Model {
      static initClass() {
        ...
        House.prototype.schema = { occupants: ['hasMany', Occupant, { as: 'livesIn' }] };
  ...
  it('5. bug fix for relational models https://github.com/kmalakoff/knockback/issues/34', function (done) {
    ...
    class Book extends Backbone.Model {
      static initClass() {
        ...
        Book.prototype.defaults = { name: 'untitled' };
      }
    }
    Book.initClass();
    // idAttribute: '_id'
    ...
    class Author extends Backbone.Model {
      static initClass() {
        ...
        Author.prototype.defaults = { name: 'untitled' };
      }
    }
    Author.initClass();
    // idAttribute: '_id'
    ...
    const bs = new BookStore({
      ...
      books: [
        { id: 'b1', name: 'Book One', author: 'a1' },
        { id: 'b2', name: 'Book Two', author: 'a1' },
      ],
      authors: [
        { name: 'fred', id: 'a1' },
        { name: 'ted', id: 'a2' },
    ...
    const queue = new Queue(1);
    for (model of Array.from(bs.get('authors').models)) {
      ((model) => queue.defer((callback) => model.save(callback)))(model);
    }
    for (model of Array.from(bs.get('books').models)) {
      ((model) => queue.defer((callback) => model.fetchRelated(callback)))(model);
    }
    return queue.await(function (err) {
      if (err) {
      ...
      for (var book of Array.from(view_model.books())) {
        var author = book.author();
        for (var authored_book of Array.from(author.books())) {
