...
describe('collection-observable @quick @collection-observable', function () {
  ...
  it('2. Basic Usage: collection observable with ko.computed', function (done) {
  ...
  it('3. Basic Usage: collection observable with ko.computed', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  it('4. Basic Usage: no view models', function (done) {
    ...
    let model_count = 0;
    _.each(collection_observable(), (_model) => model_count++);
  ...
  it('5. Basic Usage: no sorting and no callbacks', function (done) {
    ...
    _.each(collection_observable(), (_view_model) => view_model_count++);
  ...
  it('7. Collection sync sorting with comparator', function (done) {
    ...
    const sortNumber = function (model_a, model_b) {
      ...
      for (var index in parts_b) {
        var delta;
        var part = parts_b[index];
  ...
  it('12. Filters option', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
    const collection = new kb.Collection([
      { id: 1, name: 'Bob' },
      { id: 2, name: 'Fred' },
      { id: 3, name: 'George' },
    ]);
    ...
    assert.equal(
      _.map(
        _.map(collection_observable1(), (x) => x.name),
        (o) => o()
      ).join(', '),
      'Bob, Fred, George'
    );
    assert.equal(
      _.map(
        _.map(collection_observable2(), (x) => x.name),
        (o) => o()
      ).join(', '),
      'Bob'
    );
    assert.equal(
      _.map(
        _.map(collection_observable3(), (x) => x.name),
        (o) => o()
      ).join(', '),
      'Fred'
    );
    assert.equal(
      _.map(
        _.map(collection_observable4(), (x) => x.name),
        (o) => o()
      ).join(', '),
      ''
    );
    assert.equal(
      _.map(
        _.map(collection_observable5(), (x) => x.name),
        (o) => o()
      ).join(', '),
      ''
    );
    assert.equal(
      _.map(
        _.map(collection_observable6(), (x) => x.name),
        (o) => o()
      ).join(', '),
      'Bob, Fred'
    );
    assert.equal(
      _.map(
        _.map(collection_observable7(), (x) => x.name),
        (o) => o()
      ).join(', '),
      'George'
    );
    ...
    assert.equal(
      ...
      'Bob, Fred, George, Bob, Fred, George, Mary'
    );
    assert.equal(
      _.map(
        _.map(collection_observable2(), (x) => x.name),
  ...
  it('17. collection changes do not cause dependencies inside ko.computed', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  it('18. Test auto-generate collections', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
    ...
    for (var view_models of Array.from(collection_observable())) {
  ...
  it('19. Test auto-generate collections with model array', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
    ...
    for (var view_model of Array.from(collection_observable())) {
  ...
  it('20. push and unshift', function (done) {
    if (kb.Backbone && kb.Backbone.VERSION[0] !== '1') {
      return done();
    }
  ...
  it('21. Auto compact for collections', function (done) {
    let vm;
    kb.statistics = new kb.Statistics(); // turn on stats

    let models = [1, 2, 3, 4].map((id) => new Contact({ id }));
    class PersonViewModel extends kb.ViewModel {}
    ...
    assert.equal(previous_view_models.length, 4);
    for (vm of Array.from(new_view_models)) {
      if (vm.model() === collection_observable.collection().models[4]) {
        assert.ok(!Array.from(previous_view_models).includes(vm));
      } else {
        assert.ok(Array.from(previous_view_models).includes(vm));
      }
    }
  ...
  it('22. reduced form for view models', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  it('23. reduced form for view models with options', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  it('24. expanded form for collection, view models with options', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  it('24. expanded form for collection, view models with many options', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
  ...
  return it('25. Test auto-generate collections (kb.observableCollection)', function (done) {
    kb.statistics = new kb.Statistics(); // turn on stats
