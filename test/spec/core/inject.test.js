...
describe('inject @quick @inject', function () {
  let SubClass;
  let SuperClass;
  let kb = root != null ? root.kb : undefined;
  try {
    if (!kb) {
  ...
  root.appCallbacks = function (_view_model) {
    this.app = true;
    kb.statistics.register('app', this);
    this.destroy = () => kb.statistics.unregister('app', this);

    this.beforeBinding = () => {
      return (this.before_was_called = true);
    };
    this.afterBinding = () => {
      return (this.after_was_called = true);
    };
...
<div>
  ...
  <div kb-inject="app"><span data-bind="visible: app"></span></div>
    ...
    injected = kb.injectViewModels();
    ({ view_model } = injected[0]);
    const _view_model1 = injected[1].view_model;
