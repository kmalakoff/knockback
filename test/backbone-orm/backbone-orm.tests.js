...
describe('Knockback.js with BackboneORM', () => {
  ...
  it('1. Model with HasMany relations: A house with multiple people living in it', () => {
    ...
    _.each([john, paul], model => {
      assert.ok(
        kb.Statistics.eventsStats(model).count === 2,
        `All model events cleared. Expected: 2. Actual: ${JSON.stringify(kb.Statistics.eventsStats(model))}`
      );
    });
  ...
  it('4. After view model create, add models', () => {
    ...
    class House extends Backbone.Model {
      static initClass() {
        ...
        this.prototype.schema = { occupants: ['hasMany', Occupant, { as: 'livesIn' }] };
  ...
  it('5. bug fix for relational models https://github.com/kmalakoff/knockback/issues/34', done => {
    class Book extends Backbone.Model {
      static initClass() {
        ...
        this.prototype.defaults = { name: 'untitled' };
      }
    }
    Book.initClass();
    // idAttribute: '_id'
  ...
  it('8a. Customizing observable types: from the start', () => {
    ...
    assert.equal(
      view_model_house1
        ...
        .name(),
      'Home Sweet Home',
      'person1 occupies home sweet home'
    );
