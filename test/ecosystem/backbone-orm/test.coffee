module('knockback.js with BackboneORM')
...
BackboneORM = if not window.BackboneORM and (typeof(require) isnt 'undefined') then require('backbone-orm') else window.BackboneORM
...
test '3. Model with recursive HasMany relations: Person with users who are people', ->
  ...
  george.save(->); john.save(->); paul.save(->); ringo.save(->)
  george.fetchRelated(->); john.fetchRelated(->); paul.fetchRelated(->); ringo.fetchRelated(->)
...
test '8a. Customizing observable types: from the start', ->
  ...
  person1.save(->); person2.save(->)
  person1.fetchRelated(->); person2.fetchRelated(->); house.fetchRelated(->)
