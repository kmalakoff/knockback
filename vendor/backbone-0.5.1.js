...
(function () {
  ...
  _.extend(Backbone.Model.prototype, Backbone.Events, {
    ...
    set(attrs, options) {
      ...
      const alreadyChanging = this._changing;
      this._changing = true;
    ...
    // that will be called.
    url() {
      const base = getUrl(this.collection) || this.urlRoot || urlError();
  ...
  _.extend(Backbone.Collection.prototype, Backbone.Events, {
    ...
    _add(model, options) {
      ...
      model = this._prepareModel(model, options);
      if (!model) return false;
      const already = this.getByCid(model) || this.get(model);
      if (already) throw new Error(["Can't add the same model to a set twice", already.id]);
      this._byId[model.id] = model;
      this._byCid[model.cid] = model;
      const index = options.at != null ? options.at :
  ...
  const methods = ['forEach', 'each', 'map', 'reduce', 'reduceRight', 'find', 'detect',
  ...
  _.extend(Backbone.History.prototype, {
    ...
    // a `hashchange` event.
    navigate(fragment, triggerRoute) {
      let frag = (fragment || '').replace(hashStrip, '');
  ...
  // a specific view.
  const selectorDelegate = function (selector) {
  ...
  const viewOptions = ['model', 'collection', 'el', 'id', 'attributes', 'className', 'tagName'];
  ...
  _.extend(Backbone.View.prototype, Backbone.Events, {
    ...
    _configure(options) {
      ...
      for (let i = 0, l = viewOptions.length; i < l; i++) {
        const attr = viewOptions[i];
  ...
  const extend = function (protoProps, classProps) {
