...
(function (window, document, navigator, undefined) {
  ...
  !(function (factory) {
    ...
    ko.utils = new (function () {
      ...
      return {
        ...
        toggleDomNodeCssClass: function (node, className, shouldHaveClass) {
          ...
          var hasClass =
            ko.utils.arrayIndexOf(currentClassNames, className) >= 0;
        ...
        try {
          ...
          ko.dependencyDetection.begin(function (subscribable) {
            ...
            if (
              ...
              disposalCandidates[inOld] =
                undefined; // Don't want to dispose this subscription, as it's still being used
            else
    ...
    (function () {
        ...
        parseBindingsString: function (bindingsString, bindingContext) {
          try {
            var viewModel = bindingContext["$data"],
              scopes =
                typeof viewModel == "object" && viewModel != null
                  ? [viewModel, bindingContext]
                  : [bindingContext],
              bindingFunction = createBindingsStringEvaluatorViaCache(
                bindingsString,
                scopes.length,
                this.bindingCache
              );
      ...
      function createBindingsStringEvaluator(bindingsString, scopesCount) {
        var rewrittenBindings =
          ...
          " } ";
        return ko.utils.buildEvalWithinScopeFunction(
          rewrittenBindings,
          scopesCount
        );
      }
          ...
          for (var i = 0, j = value.length; i < j; i++) {
            ...
            var optionValue =
              typeof allBindings["optionsValue"] == "string"
                ? value[i][allBindings["optionsValue"]]
                : value[i];
      ...
      getSelectedValuesFromSelectNode: function (selectNode) {
        ...
        for (var i = 0, j = nodes.length; i < j; i++) {
          ...
          if (tagName == "option" && node.selected)
            ...
            var selectedValuesFromOptGroup =
              ko.bindingHandlers[
                "selectedOptions"
              ].getSelectedValuesFromSelectNode(node);
            Array.prototype.splice.apply(
              result,
              [result.length, 0].concat(selectedValuesFromOptGroup)
            ); // Add new entries to existing 'result' instance
          }
        }
        return result;
      },
      init: function (element, valueAccessor, allBindingsAccessor) {
        ko.utils.registerEventHandler(element, "change", function () {
          ...
          var valueToWrite =
            ko.bindingHandlers[
              "selectedOptions"
            ].getSelectedValuesFromSelectNode(this);
          ko.jsonExpressionRewriting.writeValueToProperty(
            value,
            allBindingsAccessor,
            "value",
    ...
    ko.templateEngine.prototype["isTemplateRewritten"] = function (
      ...
      var templateIsInExternalDocument =
        templateDocument && templateDocument != document;
      if (
        !templateIsInExternalDocument &&
    ...
    ko.templateRewriting = (function () {
      ...
      function constructMemoizedTagReplacement(
        ...
        var dataBindKeyValueArray =
          ko.jsonExpressionRewriting.parseObjectLiteral(dataBindAttributeValue);
    ...
    ko.exportSymbol(
      "templateRewriting.applyMemoizedBindingsToNextSibling",
    ...
    (function () {
      ...
      ko.renderTemplateForEach = function (
        ...
        var executeTemplateForArrayItem = function (arrayValue, index) {
          // Support selecting template as a function of the data being rendered
          var templateName =
            typeof template == "function" ? template(arrayValue) : template;
      ...
      var templateSubscriptionDomDataKey =
        "__ko__templateSubscriptionDomDataKey__";
      function disposeOldSubscriptionAndStoreNewOne(element, newSubscription) {
        var oldSubscription = ko.utils.domData.get(
          element,
          templateSubscriptionDomDataKey
        );
        ...
        update: function (
          ...
          if (typeof bindingValue == "string") {
            ...
            if ("if" in bindingValue)
              shouldDisplay =
                shouldDisplay && ko.utils.unwrapObservable(bindingValue["if"]);
            if ("ifnot" in bindingValue)
              shouldDisplay =
                shouldDisplay &&
                !ko.utils.unwrapObservable(bindingValue["ifnot"]);
          }

          var templateSubscription = null;

          if (typeof bindingValue === "object" && "foreach" in bindingValue) {
            // Note: can't use 'in' operator on strings
            ...
            if (shouldDisplay) {
              ...
              var innerBindingContext =
                typeof bindingValue == "object" && "data" in bindingValue
                  ? bindingContext["createChildContext"](
                      ko.utils.unwrapObservable(bindingValue["data"])
    ...
    (function () {
      ...
      function calculateEditDistanceMatrix(
        ...
        var distances = [];
        for (var i = 0; i <= newArray.length; i++) distances[i] = [];

        // Top row - transform old array into empty array via deletions
        for (
          var i = 0, j = Math.min(oldArray.length, maxAllowedDistance);
          i <= j;
          i++
        )
          distances[0][i] = i;
        ...
        for (
          var i = 1, j = Math.min(newArray.length, maxAllowedDistance);
          i <= j;
          i++
        ) {
          distances[i][0] = i;
        ...
        for (oldIndex = 1; oldIndex <= oldIndexMax; oldIndex++) {
          ...
          var newIndexMaxForRow = Math.min(
            newIndexMax,
            oldIndex + maxAllowedDistance
          );
          for (
            newIndex = newIndexMinForRow;
            newIndex <= newIndexMaxForRow;
            newIndex++
          ) {
            if (oldArray[oldIndex - 1] === newArray[newIndex - 1])
              distances[newIndex][oldIndex] =
                distances[newIndex - 1][oldIndex - 1];
            else {
              var northDistance =
                distances[newIndex - 1][oldIndex] === undefined
                  ? Number.MAX_VALUE
                  : distances[newIndex - 1][oldIndex] + 1;
              var westDistance =
                distances[newIndex][oldIndex - 1] === undefined
                  ? Number.MAX_VALUE
                  : distances[newIndex][oldIndex - 1] + 1;
              distances[newIndex][oldIndex] = Math.min(
                northDistance,
                westDistance
              );
            }
      ...
      function findEditScriptFromEditDistanceMatrix(
        editDistanceMatrix,
        oldArray,
        newArray
      ) {
        ...
        while (oldIndex > 0 || newIndex > 0) {
          ...
          var distanceViaAdd =
            newIndex > 0
              ? editDistanceMatrix[newIndex - 1][oldIndex]
              : maxDistance + 1;
          var distanceViaDelete =
            oldIndex > 0
              ? editDistanceMatrix[newIndex][oldIndex - 1]
              : maxDistance + 1;
          var distanceViaRetain =
            newIndex > 0 && oldIndex > 0
              ? editDistanceMatrix[newIndex - 1][oldIndex - 1]
              : maxDistance + 1;
          ...
          if (
            distanceViaAdd <= distanceViaDelete &&
            distanceViaAdd < distanceViaRetain
          ) {
            editScript.push({ status: "added", value: newArray[newIndex - 1] });
            newIndex--;
          } else if (
            distanceViaDelete < distanceViaAdd &&
            distanceViaDelete < distanceViaRetain
          ) {
            editScript.push({
              status: "deleted",
      ...
      ko.utils.compareArrays = function (
        ...
        if (maxEditsToConsider === undefined) {
          return (
            ko.utils.compareArrays(oldArray, newArray, 1) || // First consider likely case where there is at most one edit (very fast)
            ko.utils.compareArrays(oldArray, newArray, 10) || // If that fails, account for a fair number of changes while still being fast
            ko.utils.compareArrays(oldArray, newArray, Number.MAX_VALUE)
          ); // Ultimately give the right answer, even though it may take a long time
        } else {
          oldArray = oldArray || [];
          newArray = newArray || [];
          var editDistanceMatrix = calculateEditDistanceMatrix(
            oldArray,
            newArray,
            maxEditsToConsider
          );
          return findEditScriptFromEditDistanceMatrix(
            editDistanceMatrix,
            oldArray,
            newArray
          );
        ...
        for (var i = 0, j = editScript.length; i < j; i++) {
          switch (editScript[i].status) {
            ...
            case "added":
              ...
              var mapData = mapNodeAndRefreshWhenChanged(
                domNode,
                mapping,
                valueToMap,
                callbackAfterAddingNodes,
              ...
              for (
                var nodeIndex = 0, nodeIndexMax = mappedNodes.length;
                nodeIndex < nodeIndexMax;
                nodeIndex++
              ) {
    ...
    (function () {
      ko.jqueryTmplTemplateEngine = function () {
        ...
        var jQueryTmplVersion = (this.jQueryTmplVersion = (function () {
          ...
          try {
            if (
              jQuery["tmpl"]["tag"]["tmpl"]["open"].toString().indexOf("__") >=
              0
            ) {
